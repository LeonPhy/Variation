...
def guess_dR(BR, B, B_inv):
    ...
    y /= np.dot(y,y)**0.5
    print('Reddening Variation inital guess:', np.around(y,2))
...
def lagrangian(R2, l1, l2, rho, B_inv, R1, BR1, EEC, EmC, ECm, ECR, ERC, RCm, mCR, RCR):
    ...
    lagrangian = chi + constraint + penalty
...
def main():
    ...
    dir = 'data/variation/'
    test = dir + 'green2020_test_data_small.h5'
    ...
    if i == (convergence_limit-1):
        print('No convergence within {} iterations possible!'.format(convergence_limit+1)
        return 0
